...
namespace Yahvol.Web.Http.Tests
{
    ...
    public class WebCommandServiceTests
    {
        ...
        public async Task Throttle_TwoAtATime_ThirdWaitsForCompletion()
        {
            ...
            var scs = new ServiceCommandService(contextFactory, throttleInstanceFactory.Create()).AddCommand(command)
            ...
            var subscribers = scs.Subscribers;
            var serviceCommand = contextFactory.Create().ServiceCommands.First();
